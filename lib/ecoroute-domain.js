const DEFAULT_ZONE_ID = "central-market";
...
export const locationOptions = cityZones.map((zone) => ({
  id: zone.id,
  label: zone.label,
  ward: zone.ward,
}));
...
export function buildRoutePlan(reports, options = {}) {
  ...
  const startLocation = resolveLocation(options.startLocation ?? options.depot ?? cityZones.find((zone) => zone.id === "north-depot"));
